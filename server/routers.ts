  ...
  admin: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // 登录成功，清除失败记录
        delete g._adminLoginFails[lockKey];       // 签发管理员JWT token
    ...
    approveRechargeOrder: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 记录金币流水
        await insertGoldLog(order.playerId, goldToAdd, newGold, 'recharge', `充値审批到账 ${goldToAdd.toFixed(2)} 金币 (订单号: ${order.orderNo})`);
